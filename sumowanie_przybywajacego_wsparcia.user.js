...
(function() {
	...
	function date_parts_to_timestamp(date_parts) {
		...
		var timestampUTC = Date.UTC(year, date_parts.month - 1, date_parts.day, date_parts.hour, date_parts.minute, date_parts.second);
		return Math.floor(timestampUTC / 1000);
	}

	function string_to_timestamp(input) {
		var now = new Date();
    ...
    var timeParts = parts[parts.length - 1].split(":");
    var hour = parseInt(timeParts[0], 10);
    var minute = parseInt(timeParts[1], 10);
    var second = parseInt(timeParts[2], 10);

    // GMT+2 offset in minutes
    var timezoneOffset = 2 * 60;

    if (parts[0] === "jutro") {
        // Tomorrow
        var tomorrow = new Date(year, month, day + 1, hour, minute, second);
        return Math.floor(tomorrow.getTime() / 1000) - timezoneOffset * 60;
    } else if (parts[0] === "dzisiaj") {
        // Today
        var today = new Date(year, month, day, hour, minute, second);
        return Math.floor(today.getTime() / 1000) - timezoneOffset * 60;
    } else {
        // Parse date after "dnia"
        var dateIndex = parts.indexOf("dnia");
        if (dateIndex !== -1 && dateIndex + 3 < parts.length) {
            var customDay = parseInt(parts[dateIndex + 1], 10);
            var customMonth;
            
            if (parts[dateIndex + 2].includes(".")) {
                // Handle "dd.mm." format
                customMonth = parseInt(parts[dateIndex + 2].split(".")[1], 10) - 1;
            } else if (parts[dateIndex + 2].includes(":")) {
                // Handle "dd.mm o hh:mm:ss" format
                customMonth = month;
                customDay = day;
            } else {
                // Handle "dd.mm" format
                customMonth = parseInt(parts[dateIndex + 2], 10) - 1;
            }

            var customDate = new Date(Date.UTC(year, customMonth, customDay, hour, minute, second));
            return Math.floor(customDate.getTime() / 1000) - timezoneOffset * 60;
        } else {
            // Invalid format
            console.error("Invalid date format after 'dnia'");
            return NaN;
        }
    }
	}
